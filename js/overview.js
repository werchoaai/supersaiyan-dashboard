/**
 * Overview Page — stats cards, phase diagram, task table with filters
 */

function renderOverview() {
  const store = Alpine.store ? Alpine.store('dashboard') : window._dashboardStore;
  ...
  // Phase counts (for open tasks only)
  const phaseCounts = {};
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-left">
          ...
          <tbody>
            ${filtered.length === 0 ? '<tr><td colspan="7" class="py-8 text-center text-gray-400 text-sm">No tasks match your filters.</td></tr>' : ''}
  ...
  return `
    <th class="py-2.5 px-4 sort-header" onclick="window._updateSort('${key}')">
      <span class="inline-flex items-center gap-1">
        ${label}
        <span class="text-gray-300" id="sort-icon-${key}">&#8597;</span>
      </span>
    </th>
  `;
}
...
function taskRow(task) {
  ...
  const created = task.createdAt ? new Date(task.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—';
  const agentColors = {
    claude: 'text-indigo-600',
  ...
  return `
    <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer"
      ...
      <td class="py-3 px-4">
        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium status-${task.status}">
          ${task.status}
        </span>
      </td>
      <td class="py-3 px-4">
        <div class="font-medium text-sm text-gray-900 hover:text-indigo-600">${escapeHtml(task.title || task.id)}</div>
        <div class="text-xs text-gray-400 font-mono">${task.id}</div>
      </td>
      <td class="py-3 px-4">
        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium phase-${phase}">
          ${phase.replace('_', ' ')}
        </span>
      </td>
      <td class="py-3 px-4">
        <span class="text-sm font-medium ${agentColors[task.nextAgent] || 'text-gray-400'}">
          ${task.nextAgent || '—'}
        </span>
      </td>
